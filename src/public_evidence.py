...
def _dataset_resources(dataset_id):
    urls=[];meta=_json("https://data.gov.tw/api/v2/rest/dataset/{}".format(dataset_id)); distributions=meta.get("distribution") or meta.get("distributions") or []
...
def _evidence(source,record_id,title,summary,url,raw,fact_type,matched_terms=None):
    now=datetime.now(timezone.utc).isoformat();return {"evidence_id":"{}:{}".format(source,record_id),"schema_version":"1.0","observed_at":now,"retrieved_at":now,"source":{"type":"government_open_data","name":source,"record_id":str(record_id),"url":url},"fact":{"type":fact_type,"title":title,"summary":summary,"matched_terms":matched_terms or []},"confidence":1.0,"status":"active","raw":raw}
...
def _judicial_recent(needles,max_docs=50):
    ...
    try:
        def post(path,payload,timeout=30):
            body=json.dumps(payload,ensure_ascii=False).encode("utf-8");req=urllib.request.Request("https://data.judicial.gov.tw/jdg/api"+path,data=body,headers={"Content-Type":"application/json","User-Agent":"TaiwanEntityIntelligence/0.5"});return json.loads(urllib.request.urlopen(req,timeout=timeout).read().decode("utf-8"))
...
def collect_public_evidence(company_name,people=None):
    needles=[_norm(company_name)]+[_norm(p) for p in (people or []) if p];needles=[n for n in needles if len(n)>=2];evidence=[];statuses={}
