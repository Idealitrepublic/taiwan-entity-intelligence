...
def _demo_response():
    ...
    for pid, name, role in people:
        nodes.append({"id":pid,"type":"person","label":name,"properties":{"position":role,"source":"official_judgment"}})
        edges.append({"source":"company:82876417","target":pid,"relationship":role})
    evidence = [
        {
            "evidence_id":"judgment:KLDM-111訴328","schema_version":"1.0",
            "source":{"type":"judicial_public","name":"司法院裁判書－111年度訴字第328號","record_id":"KLDM,111,訴,328,20230428,1","url":"https://judgment.judicial.gov.tw/FJUD/printData.aspx?id=KLDM%2C111%2C%E8%A8%B4%2C328%2C20230428%2C1"},
            "fact":{"type":"judgment","title":"臺灣基隆地方法院 111年度訴字第328號","summary":"判決記載吳虹葳以人頭負責人及股東身分設立御首公司，並記載詐欺集團以御首公司名義對21人實施詐欺。此為裁判書觀測事實，不代表公司法人本身當然成立刑事責任。"},
        ...
        },
        {
            ...
            "source":{"type":"judicial_public","name":"司法院裁判書－110年度原訴字第9號","record_id":"KLDM,110,原訴,9,20220128,11","url":"https://data.judicial.gov.tw/opendl/JDocFile/KLDM/110%2C%E5%8E%9F%E8%A8%B4%2C9%2C20220128%2C11.pdf"},
            "fact":{"type":"judgment","title":"臺灣基隆地方法院 110年度原訴字第9號","summary":"判決內容多處直接提及御首公司、林宇澤及相關人員的角色與案件事實；應以裁判全文核對各人的最終罪責與判決主文。"},
            "confidence":1.0,"status":"active","entity_id":"company:82876417","entity_type":"company"
        }
    ]
    return {
        "uniform_number":DEMO_COMPANY,
        "company":{"Company_Name":company,"Responsible_Name":"吳虹葳","Company_Status":"解散","Company_Setup_Date":"108/05/17","Capital_Stock_Amount":"200,000"},
        "company_name":company,
        "people":[{"person_name":n,"position":r} for _,n,r in people],
        "graph":{"nodes":nodes,"edges":edges},
        "data_mode":"demo_judicial_case",
        "evidence":evidence,
        "evidence_count":len(evidence),
        "evidence_sources":{"司法院裁判書－111年度訴字第328號":1,"司法院裁判書－110年度原訴字第9號":1},
        "evidence_status":{"標案":"not_run","裁罰":"not_run","165":"not_run","裁判書":"demo_fixture"},
        "judicial_search_url":JUDICIAL_SEARCH.format(quote_plus(company)),
        "evidence_note":"這是官方裁判書案例的測試 fixture，用來驗證圖譜與 Evidence UI；不是即時全量裁判書查詢。"
    }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not uniform_number.isdigit() or len(uniform_number)!=8:
            self._json(400,{"error":"統編必須是 8 碼數字。"}); return
        try:
            params=parse_qs(parsed.query)
            if uniform_number==DEMO_COMPANY and params.get("demo",["0"])[0].lower() in ("1","true","yes"):
            ...
            try:
                ...
                people=[{"uniform_number":uniform_number,"company_name":(basic or {}).get("Company_Name") or uniform_number,"position":n.get("properties",{}).get("position"),"person_name":n.get("label"),"shares":n.get("properties",{}).get("shares")} for n in graph.get("nodes",[]) if n.get("type")=="person"]
            ...
            if not basic and not people:
                conn.close(); self._json(404,{"error":"找不到此統編。"}); return
            company_name=(basic or {}).get("Company_Name") or (people[0]["company_name"] if people else uniform_number)
            graph=company_graph(conn,uniform_number); tenders=company_tenders(conn,company_name); conn.close()
            public=collect_public_evidence(company_name,[p.get("person_name") for p in people])
