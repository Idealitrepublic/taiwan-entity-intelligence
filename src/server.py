...
WORKING_DEPLOYMENT = os.environ.get("WORKING_DEPLOYMENT", "https://taiwan-entity-intelligence-641h-coldlight871029-9944.vercel.app")


def _demo_response():
    company = "御首服務事業有限公司"
    people = [
        ("person:林宇澤", "林宇澤", "主要聯絡／業務運作人物"),
        ("person:吳虹葳", "吳虹葳", "判決記載之人頭負責人及股東"),
        ("person:王凱迪", "王凱迪", "判決記載之出資人"),
        ("person:周金川", "周金川", "判決記載之相關人物"),
        ("person:賴乙誠", "賴乙誠", "判決記載之相關人物"),
        ("person:朱家禾", "朱家禾", "判決記載之相關人物"),
    ]
    nodes = [{"id": "company:82876417", "type": "company", "label": company,
              "properties": {"uniform_number": "82876417", "demo": True, "source_note": "官方裁判書案例測試資料"}}]
    edges = []
    for pid, name, role in people:
        nodes.append({"id": pid, "type": "person", "label": name,
                      "properties": {"position": role, "source": "official_judgment"}})
        edges.append({"source": "company:82876417", "target": pid, "relationship": role})
    base_evidence = [
        {"evidence_id": "judgment:KLDM-111訴328", "schema_version": "1.0",
         "source": {"type": "judicial_public", "name": "司法院裁判書－111年度訴字第328號", "record_id": "KLDM,111,訴,328,20230428,1",
                    "url": "https://judgment.judicial.gov.tw/FJUD/printData.aspx?id=KLDM%2C111%2C%E8%A8%B4%2C328%2C20230428%2C1"},
         "fact": {"type": "judgment", "title": "臺灣基隆地方法院 111年度訴字第328號",
                  "summary": "判決記載吳虹葳以人頭負責人及股東身分設立御首公司，並記載詐欺集團以御首公司名義對21人實施詐欺。此為裁判書觀測事實，不代表公司法人本身當然成立刑事責任。"},
         "confidence": 1.0, "status": "active", "entity_id": "company:82876417", "entity_type": "company"},
        {"evidence_id": "judgment:KLDM-110原訴9", "schema_version": "1.0",
         "source": {"type": "judicial_public", "name": "司法院裁判書－110年度原訴字第9號", "record_id": "KLDM,110,原訴,9,20220128,11",
                    "url": "https://data.judicial.gov.tw/opendl/JDocFile/KLDM/110%2C%E5%8E%9F%E8%A8%B4%2C9%2C20220128%2C11.pdf"},
         "fact": {"type": "judgment", "title": "臺灣基隆地方法院 110年度原訴字第9號",
    ...
    try:
        ...
        note = "Demo 案例 + 即時公開資料查詢。Demo 判決為固定案例；其餘來源狀態與命中數以即時查詢結果為準。"
    except Exception as exc:
        live_evidence, statuses = [], {"資料源總數": {"status": "error", "message": str(exc)}}
        note = "Demo 案例；即時公開資料查詢失敗，請查看來源狀態。"
    evidence = base_evidence + live_evidence
    return {
        "uniform_number": DEMO_COMPANY, "company": {"Company_Name": company, "Responsible_Name": "吳虹葳", "Company_Status": "解散", "Company_Setup_Date": "108/05/17", "Capital_Stock_Amount": "200,000"},
        "company_name": company, "people": [{"person_name": n, "position": r} for _, n, r in people],
        "graph": {"nodes": nodes, "edges": edges}, "data_mode": "demo_judicial_case_live_sources",
        "evidence": evidence, "evidence_count": len(evidence),
        "evidence_sources": {k: sum(1 for x in evidence if x.get("source", {}).get("name") == k) for k in sorted(set(x.get("source", {}).get("name", "其他") for x in evidence))},
        "evidence_status": {"裁判書": {"status": "demo_fixture_plus_live_check", "matched": len(live_evidence) + len(base_evidence)}, **statuses},
        "judicial_search_url": JUDICIAL_SEARCH.format(quote_plus(company)), "evidence_note": note,
    }
...
def _decorate_evidence(evidence, company_name, people, uniform_number):
    ...
    for item in evidence:
        ...
        if cn and cn in hay:
            item['entity_id'] = 'company:{}'.format(uniform_number); item['entity_type'] = 'company'
        else:
            for norm, name in pn:
                if len(norm) >= 2 and norm in hay:
                    item['entity_id'] = 'person:{}'.format(name); item['entity_type'] = 'person'; break
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/':
            ...
            injection = b'<script src="/tei-enhancements.js?v=3"></script>'
