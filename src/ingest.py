...
def ingest(db_path: str, path: str, source_key: str, entity_field: str, record_id_field: str,
           target_field: str = "", title_field: str = "", date_field: str = "", url_field: str = "") -> int:
    ...
    try:
        for row in load_records(path):
            ...
            evidence = make_evidence(
                source_type=preset["source_type"], source_name=preset["source_name"],
                source_record_id=record_id, entity_id=entity_id,
                entity_type="company" if entity_id.startswith("company:") else "entity",
                fact_type=preset["fact_type"], relation_type="source_record",
                target_entity_id=target_id, target_entity_type="entity" if target_id else None,
                ...
                confidence=1.0, raw_payload=row,
            )
...
def main() -> None:
    ...
    options = vars(args)
    source_key = options.pop("source")
    count = ingest(source_key=source_key, **options)
