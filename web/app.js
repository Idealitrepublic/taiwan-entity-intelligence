...
async function loadStatus() {
  try {
    ...
    const db = $("dbStatus");
    if (x.supabase && x.supabase.configured) {
      db.innerHTML = `
...
function renderCompany(x) {
  ...
  if (hits.length) {
    ...
    $("signal").innerHTML = `<b>你的資料庫命中</b><br>${hits.map(esc).join("<br>")}`;
  } else {
    $("signal").style.display = "block";
    $("signal").innerHTML = `<b>目前資料層</b><br>本企業尚無結構化 Supabase evidence；以下內容以即時公開資料為主。`;
  }
...
function renderEvidence(rows) {
  ...
  $("evidencePane").innerHTML = rows.map((e,i) => {
...
async function searchCompany() {
  ...
  try {
    const r=await fetch(`/api/company/${q}?ts=${Date.now()}`); const x=await r.json();
